...
| 工具 / 资源 | 归入 | [`tools/`](tools/) | `tools/runelf.hpappdir/`、`tools/fonts/` |
...
| `tools/runelf.hpappdir/` | 通用 ELF 运行器（加载器 + 示例 `my_app.elf`） | 本身即 `.hpappdir` | 计算器工具 |
| `tools/fonts/` | Prime Sans 字体（固件提取，含 CJK） | — | 资源 |
...
3. 在顶层目录下建 `<name>.hpappdir/`，至少包含：
   - `main.py`：计算器端加载器（ELF 运行器可直接复用 [`tools/runelf.hpappdir/main.py`](tools/runelf.hpappdir/main.py)，改 `APP_ELF_FILENAME` / `APP_DIR_FOR_C_CODE` 两个常量）；
...
- 名称依据**源码实际内容**：`primecode.c` 是编辑器、`suika_prime.c` 是水果游戏
  （两者历史上曾被混放在同一目录）；
- 结构：演示应用入 `examples/`，单仓库候选入 `staging/`（前缀），工具与资源入 `tools/`；
- 抢救自废弃仓库：`FileManager.hpappdir/`、`tools/fonts/`（Prime Sans）；
